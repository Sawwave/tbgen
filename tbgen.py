...
#Modified May 2018, Tim Anderson
...
class TestbenchGenerator(object):
    ...
    def parser(self):
        print "Parsering..."
        # print vf_cont
        module_regex = string(r"[\s]+module[\s](\S*)" //match module name
        "[\s]*(#[\s]*\([^\)]*\))?"  //match module parameters
        "([\s]*\([^;]*);")          //match module ports
        module_result = re.findall(mod_pattern, self.clean_other(self.vcont))
        #print module_result
        self.mod_name = module_result[0]
        self.mod_params = parse_mod_params(module_results[1])
        self.mod_ports = parse_mod_ports(module_results[2])

        self.parser_inoutput()
        self.find_clk_rst()


    def parse_mod_params(self, param_string):
        raise NotImplementedError
        
    def parse_mod_ports(self, port_string):
        raise NotImplementedError
